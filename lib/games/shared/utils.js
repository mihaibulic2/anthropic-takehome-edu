...
window.GameUtils = (() => {
  ...
  // Track answer attempt and question history for adaptive learning
  function trackAnswer(question, userAnswer, isCorrect) {
    // Update game stats
    ...
    const correctAnswer = question.answers ? question.answers[question.correctAnswerIndex] : question.correctAnswer;
    ...
    return new Promise((resolve) => {
      try {
        ...
        const handleMessage = (event) => {
          if (event.data.type === 'GENERIC_LLM_RESPONSE' && event.data.requestId === requestId) {
            window.removeEventListener('message', handleMessage);
            resolve(event.data.response || "");
          } else if (event.data.type === 'GENERIC_LLM_RESPONSE_ERROR' && event.data.requestId === requestId) {
            window.removeEventListener('message', handleMessage);
            console.error('Error getting LLM response:', event.data.error);
            resolve("");
          }
        };
        ...
        setTimeout(() => {
          window.removeEventListener('message', handleMessage);
          console.error('LLM response timed out');
          resolve([]);
        }, 30000);
  ...
  // Generate questions using LLM API via postMessage to parent
  async function generateQuestions(gameProps, count, formatSpec) {
    return new Promise((resolve) => {
      try {
        const requestId = Math.random().toString(36).substring(7);
  ...
  // Renamed for clarity: send final stats when game ends
  function sendStats(completed = false) {
    sendGameStats(completed);
  }
  ...
  return {
    ...
    sendStats: sendStats,
    sendGameStats: sendGameStats, // Keep for backwards compatibility
