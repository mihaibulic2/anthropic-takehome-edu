...
window.GameUtils = (() => {
  ...
  function getCurrentStats() {
    ...
    const questionsAsked = gameStats.questionsAttempted;
    ...
    let review = questionHistory.map((entry, index) => {
      return `\n${index + 1}. "${entry.question}"\n\tAnswer: "${entry.correctAnswer}" (${entry.wasCorrect ? 'Correct!' : `Wrong, you said: "${entry.userAnswer}"`})`;
    }).join('\n');
